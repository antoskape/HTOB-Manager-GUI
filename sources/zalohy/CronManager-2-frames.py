...
class MainGui(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        tk.Button(master, text="Start HTOB", fg='#2f6725', font=helv10, command = self.cronStart).grid(row=12,column=0,sticky="ew")
        tk.Button(master, text="Stop HTOB", fg='#d90000', font=helv10, command = self.cronStop).grid(row=12,column=1,sticky="ew")
        tk.Button(master, text="Stop AC Power", bg='#d90000', fg='#ffffff', font=helv10, command = self.stopAC).grid(row=12,column=2,sticky="ew")
        tk.Button(master, text="Reset counters", font=helv10, command = self.resetCounter).grid(row=12,column=3,sticky="ew")
        tk.Button(master, text="Export Logs", font=helv10, command = self.exportLogs).grid(row=12,column=4,sticky="ew")
        # Quit HTOB Manager application
        tk.Button(master, text="Quit", font=helv10, command = self.closeWindows).grid(row=12,column=5,sticky="ew")
        # //////////////////////////////////////////////////////////////////////////////////
        # Left columns
        Frame1 = tk.Frame(master, bg="#dbe9c9",padx=10,pady=10)
        Frame1.grid(row=0,column=0, rowspan=7, columnspan=2, sticky="nsew")
        ...
        Frame2.grid(row=7, column=0,rowspan=2,columnspan=2,sticky="nsew")
        ...
        Frame3.grid(row=0,column=2, rowspan=3, columnspan=5,sticky="nsew")
        self.logs_path_label = tk.Label(Frame3, height=1,bg="#e6e6e6",text="LOG FILES DIRECTORY: ").grid(column=0, row=0, sticky="w")
        self.logs_path = tk.Entry(Frame3,width=60)
        self.logs_path.insert(0, os.path.join(os.path.abspath(os.path.dirname(__file__)), "log-data-backups"))
        self.logs_path.grid(column=0, row=1)
        tk.Button(Frame3,text="Choose dir",command = self.chooseDirectory).grid(row=2,column=0,sticky="w")
        # Variacs block
        # //////////////////////////////////////////////////////////////////////////////////
        Frame4 = tk.Frame(master, bg="#e6e6e6", padx=10, pady=10)
        Frame4.grid(row=3, column=2, rowspan=7, columnspan=5, sticky="nsew")
        self.variacs_title = tk.Label(Frame4, height=1, font=bold, text="AC VARIABLE TRANSFORMERS").grid(column=0, row=0, sticky="w")
        self.variac_ip_label = tk.Label(Frame4, height=1, text="IP ADDRESS").grid(column=0, row=1, sticky="w")
        self.variac_required_volt_label = tk.Label(Frame4, height=1, text="REQ.VOLTAGE(VAC)").grid(column=1, row=1, sticky="w")
        self.variac_L1_label = tk.Label(Frame4, height=1, text="L1(VAC)").grid(column=2, row=1, sticky="w")
        self.variac_L2_label = tk.Label(Frame4, height=1, bg="#e6e6e6", text="L2(VAC)").grid(column=3, row=1, sticky="w")
        self.variac_L3_label = tk.Label(Frame4, height=1, bg="#e6e6e6", text="L3(VAC)").grid(column=4, row=1, sticky="w")
        # Variac 1
        self.variac1_request = tk.Entry(Frame4, justify="left")
        self.variac1_request.grid(column=0, row=2, sticky="w")
        self.variac1_L1_voltage = tk.Entry(Frame4, justify="left")
        self.variac1_L1_voltage.grid(column=1, row=2, sticky="w")
        self.variac1_L2_voltage = tk.Entry(Frame4, justify="left")
        self.variac1_L2_voltage.grid(column=2, row=2, sticky="w")
        self.variac1_L3_voltage = tk.Entry(Frame4, justify="left")
        self.variac1_L3_voltage.grid(column=3, row=2, sticky="w")
        # Variac 2
        self.variac2_request = tk.Entry(Frame4, justify="left")
        self.variac2_request.grid(column=0, row=3, sticky="w")
        self.variac2_L1_voltage = tk.Entry(Frame4, justify="left")
        self.variac2_L1_voltage.grid(column=1, row=3, sticky="w")
        self.variac2_L2_voltage = tk.Entry(Frame4, justify="left")
        self.variac2_L2_voltage.grid(column=2, row=3, sticky="w")
        self.variac2_L3_voltage = tk.Entry(Frame4, justify="left")
        self.variac2_L3_voltage.grid(column=3, row=3, sticky="w")
        # Variac 3
        self.variac3_request = tk.Entry(Frame4, justify="left")
        self.variac3_request.grid(column=0, row=4, sticky="w")
        self.variac3_L1_voltage = tk.Entry(Frame4, justify="left")
        self.variac3_L1_voltage.grid(column=1, row=4, sticky="w")
        self.variac3_L2_voltage = tk.Entry(Frame4, justify="left")
        self.variac3_L2_voltage.grid(column=2, row=4, sticky="w")
        self.variac3_L3_voltage = tk.Entry(Frame4, justify="left")
        self.variac3_L3_voltage.grid(column=3, row=4, sticky="w")
        # Variac 4
        self.variac4_request = tk.Entry(Frame4, justify="left")
        self.variac4_request.grid(column=0, row=5, sticky="w")
        self.variac4_L1_voltage = tk.Entry(Frame4, justify="left")
        self.variac4_L1_voltage.grid(column=1, row=5, sticky="w")
        self.variac4_L2_voltage = tk.Entry(Frame4, justify="left")
        self.variac4_L2_voltage.grid(column=2, row=5, sticky="w")
        self.variac4_L3_voltage = tk.Entry(Frame4, justify="left")
        self.variac4_L3_voltage.grid(column=3, row=5, sticky="w")
        # Save button
        tk.Button(Frame4, text="Save Variacs configuration", command=self.saveVariacs()).grid(row=6, column=0, sticky="w")
        # //////////////////////////////////////////////////////////////////////////////////
    ...
    def resetCounter(self):
        try:
            ...
            self.put_config_data("TEST PARAMETERS", "start_of_test", self.start_test.get())
        except Exception:
    ...
    def startupFunction(self):
        ...
        self.start_test.insert(0, self.get_config_data('TEST PARAMETERS', 'start_of_test'))
    ...
    def cyclesToMinutes(self):
        ...
        timers_minutes = []
        # Read input parameters from .ini file - cycle duration in minutes
        cycle_interval = self.get_config_data('APPLICATION', 'test_cycle_duration')
    ...
    def get_config_data(self, section, ini_parameter):
        # Create object to get configuration data from .ini file
        config = configparser.ConfigParser()
        # Read .ini file
        config.read("CronManager-config.ini")
        # Read config data for this load
        ini_value = config.get(str(section), str(ini_parameter))
        # Return value from .ini file
        return ini_value


    # Auxiliary function - save record into configuration data .ini file
    def put_config_data(self, ini_section, ini_parameter, ini_value):
        # Create object to get configuration data from .ini file
        config = configparser.ConfigParser()
        # Read .ini file
        config.read("CronManager-config.ini")
        # Prepeare the .ini file content
        config.set(ini_section, ini_parameter, ini_value)
...
def main():
    ...
    root = tk.Tk()
    root.geometry("650x300+70+70")
    root.minsize(width=460, height=140)
    # root = root window
