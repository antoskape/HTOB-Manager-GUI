'''
HTOB CRONTAB MANAGER

Description:
HTOB Crontab Manager (GUI) provides basic functionalities to control HTOB test. It is based
on Tkinter GUI library.
Rel.01
Release date: 17/08/2020
Author: petantsk

History:
Release 01 (17/08/2020):
- first release,
- Variacs section is not working in this pre-release.
'''
...
class MainGui(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        # Constants
        self.C_RUNNING = "#558a45"   # HTOB is running, bckg color for Frame2
        self.C_STOPPED = "#727272"   # HTOB is stopped, bckg color for Frame2
        self.C_ERROR = "#e6614a"     # Error in HTOB scripts occured, bckg color for Frame2
        ...
        # Global variable for list of items in Manual Scripts-Listbox (to avoid multiple reading from .ini file)
        self.manual_scripts_dict = {}
        ...
        helv10bold= Font(family='Helvetica', size=10, weight='bold')
        self.headbold = Font(family='TkHeadingFont', size=9, weight='bold')
        giant = Font(family='TkHeadingFont', size=24, weight='bold')

        # Global variables inititation
        # Status of HTOB test - to keep actual status of test (Frame2 bckg color)
        self.htob_status = None     # HTOB is running = True/ stopped = False

        # Working directory path setup for Linux OS
        #os.chdir('/home/pi/Documents/HTOB')
        #os.chdir(self.getConfigData('APPLICATION', 'root_path'))

        # //////////////////////////////////////////////////////////////////////////////////
        # Show control buttons (Menu)
        tk.Button(master, text="Start HTOB", fg='#2f6725', bg='#faf39d', font=helv10, command = self.cronStart).grid(row=14,column=0, ipady=7, sticky="ew")
        tk.Button(master, text="Stop HTOB", fg='#d90000', bg='#faf39d', font=helv10, command = self.cronStop).grid(row=14,column=1, ipady=7, sticky="ew")
        tk.Button(master, text="Reset errors", bg='#faf39d', font=helv10, command = self.resetErrors).grid(row=14,column=2, ipady=7, sticky="ew")
        tk.Button(master, text="Reset counters", bg='#faf39d', font=helv10, command = self.resetCounter).grid(row=14, column=3, ipady=7, sticky="ew")
        tk.Button(master, text="Export Logs", bg='#faf39d', font=helv10, command = self.exportLogs).grid(row=14, column=4, ipady=7, sticky="ew")
        tk.Button(master, text="Show errors", bg='#faf39d', font=helv10, command=self.modalErrors).grid(row=14, column=5, ipady=7, sticky="ew")
        # Quit HTOB Manager application
        tk.Button(master, text="Quit", bg='#faf39d', font=helv10, command = self.closeWindows).grid(row=14, column=6, ipady=7, sticky="ew")
        # //////////////////////////////////////////////////////////////////////////////////
        # Left columns
        ...
        self.reset_cycles_label = tk.Label(Frame1, height=1,bg="#dbe9c9", font=self.headbold, text="RESET COUNTER VALUES").grid(column=0, pady=10, row=4, sticky="ew")
        ...
        # //////////////////////////////////////////////////////////////////////////////////
        self.Frame2 = tk.Frame(master, bg=self.C_STOPPED, padx=10, pady=10)
        self.Frame2.grid(row=12, column=0, rowspan=2,columnspan=2, sticky="nsew")
        self.clock = tk.Label(self.Frame2, height=1, bg=self.C_STOPPED,fg="#ffffff", font=giant,padx=40, pady=30)
        self.clock.grid(column=0, row=0)
        self.cron_status_label = tk.Label(self.Frame2, bg=self.C_STOPPED, fg="#ffffff",text="TEST STATUS\n(updated each 5s): ")
        self.cron_status_label.grid(column=0, row=1, sticky="w")
        self.cron_status = tk.Label(self.Frame2, font=helv10bold, bg=self.C_STOPPED,fg="#ffffff", text="no value available")
        self.cron_status.grid(column=1, row=1)
        # //////////////////////////////////////////////////////////////////////////////////
        Frame3 = tk.Frame(master, bg="#e6e6e6", padx=10, pady=10)
        Frame3.grid(row=0,column=2, rowspan=14, columnspan=5,sticky="nsew")
        self.logs_path_label = tk.Label(Frame3, font=self.headbold, height=1, bg="#e6e6e6", text="LOG FILES DIRECTORY: ")
        self.logs_path_label.grid(column=0, row=0, columnspan=5, sticky="w")
        self.logs_path = tk.Entry(Frame3, width=100, justify="left")
        ...
        # Variacs block
        self.variacs_title = tk.Label(Frame3, height=1, font=self.headbold, bg="#e6e6e6", text="AC VARIABLE TRANSFORMERS").grid(pady=10, column=0, row=3, columnspan=3, sticky="w")
        ...
        # Manual scripts
        self.variacs_title = tk.Label(Frame3, height=1, font=self.headbold, bg="#e6e6e6", text="RUN SCRIPTS MANUALLY").grid(pady=10, column=0, row=10, sticky="w")
        self.manual_scripts = tk.Listbox(Frame3, width=60)
        self.manual_scripts.bind('<<ListboxSelect>>', self.onSelectScript)
        self.manual_scripts.grid(row=11, column=0, columnspan=2, sticky="w")
        self.manual_scripts_scrollbar = tk.Scrollbar(Frame3)  # height= not permitted here!
        self.manual_scripts.config(yscrollcommand=self.manual_scripts_scrollbar.set)
        self.manual_scripts_scrollbar.config(command=self.manual_scripts.yview)
        self.manual_scripts_scrollbar.grid(column=1, row=11, sticky='nse')
        self.maunal_scripts_label = tk.Label(Frame3, height=1, bg="#e6e6e6", text="SELECTED SCRIPT: ").grid(pady=10, column=0, row=12, sticky="w")
        self.manual_scripts_command = tk.Label(Frame3, height=1, bg="#d6d6d6", text="no script selected")
        self.manual_scripts_command.grid(pady=10, column=1, row=12, columnspan=4, sticky="w")
        tk.Button(Frame3, text="Execute the script", command=self.executeScript).grid(row=13, column=0, sticky="w")
        # //////////////////////////////////////////////////////////////////////////////////
    ...
    def cronStart(self):
        if messagebox.askokcancel("CRONTAB JOB", "You are going to start the CRONTAB job what will run the HTOB test.\nAre you sure?"):
            try:
                ...
                file_object.close()
                # Set Frame2 bckg color
                self.Frame2.config(bg=self.C_RUNNING)
                self.clock.config(bg=self.C_RUNNING)
                self.cron_status_label.config(bg=self.C_RUNNING)
                self.cron_status.config(bg=self.C_RUNNING)
                messagebox.showinfo("CRONTAB JOB", "HTOB test was successfully started. CRONTAB file content was updated.")
            except:
                errordetection.writeError()
                messagebox.showinfo("CRONTAB JOB", "HTOB start procedure failed! Check manually if test is running.")


    def cronStop(self):
        if messagebox.askokcancel("CRONTAB JOB", "You are going to stop HTOB test. All running processes will be killed.\nAre you sure?"):
            try:
                cmd = os.popen('sudo cp /etc/crontab-stop /etc/crontab')
                # Kill all HTOB processes/scripts
                cmd = os.popen("sudo pkill -9 -f '\/HTOB-.+\.py'")
                StatArray = ["STOPPED"]  # update the status
                self.htob_status = False
                # ----------------------------------------------
                # Update CRONTAB job status in txt-file
                # ----------------------------------------------
                file_object = open("cron-status.txt", "w")  # opening the counter file for writing
                ...
                self.exportLogs()
                # Set Frame2 bckg color
                self.Frame2.config(bg=self.C_STOPPED)
                self.clock.config(bg=self.C_STOPPED)
                self.cron_status_label.config(bg=self.C_STOPPED)
                self.cron_status.config(bg=self.C_STOPPED)
                messagebox.showinfo("CRONTAB JOB", "HTOB test was successfully stopped. CRONTAB file content was cleared.")
            except:
                errordetection.writeError()
                messagebox.showinfo("CRONTAB JOB", "HTOB stop procedure failed! Check manually if test is still running.")
    ...
    def actualTimers(self):
        ...
        # End of test - calculated from remaining number of cycles
        cycles_total_minutes = date_time_now + delta_remaining
    ...
    # Reset error status to errorless mode: delete red back color, empty error-detection.txt file
    def resetErrors(self):
        if messagebox.askokcancel("RESET ERRORS", "You are going to clear Error counter for HTOB test!\nAre you sure?"):
            # error-detection.txt file content with error records is cleared
            self.deleteContent("error-detection.txt")


    # HTOB test is running/stopped
    def cronStatus(self):
        ...
        # Initiate CRON status background color for Frame2
        if value[0] == "RUNNING":
            self.htob_status = True


    # Any error arised in scripts/GUI?
    def errorStatus(self):
        # Is error records file empty?
        if not (os.stat("error-detection.txt").st_size == 0):
            # Frame2 bckg color is red if an error is registered
            self.Frame2.config(bg=self.C_ERROR)
            self.clock.config(bg=self.C_ERROR)
            self.cron_status_label.config(bg=self.C_ERROR)
            self.cron_status.config(bg=self.C_ERROR)
        # HTOB test is running (True) without an error
        elif self.htob_status:
            # Frame2 bckg color is green
            self.Frame2.config(bg=self.C_RUNNING)
            self.clock.config(bg=self.C_RUNNING)
            self.cron_status_label.config(bg=self.C_RUNNING)
            self.cron_status.config(bg=self.C_RUNNING)
        # HTOB test is not running (False)
        else:
            # Frame2 bckg color is gray
            self.Frame2.config(bg=self.C_STOPPED)
            self.clock.config(bg=self.C_STOPPED)
            self.cron_status_label.config(bg=self.C_STOPPED)
            self.cron_status.config(bg=self.C_STOPPED)
    ...
    def manualScriptsFillListbox(self):
        # Save dictionary with all scripts commands from ini.file to class variable self.manual_scripts_dict
        self.manual_scripts_dict = self.listConfigData("MANUAL SCRIPTS")
        # Fill Listbox with .ini records (dictionary)
        # script label = key
        # script command = value
        for key in self.manual_scripts_dict:
            self.manual_scripts.insert(tk.END, key)


    # Initiate Listbox with list of scripts which can be triggered manually
    def executeScript(self):
        # Check if user selected any Listbox item (script to execute)
        if (self.manual_scripts_command["text"] == " " or self.manual_scripts_command["text"] == "no script selected"):
            messagebox.showerror("MANUAL SCRIPTS", "Command is empty string! Select any command from listbox first.")
            return()

        if messagebox.askokcancel("MANUAL SCRIPTS", "You have selected a script to execute it.\nDo you want to run it?"):
            # Execute the script listed in Listbox and selected by user
            try:
                #print(self.manual_scripts_command["text"])
                exec(self.manual_scripts_command["text"])
            except:
                # Catch all exception
                e = sys.exc_info()[0]
                messagebox.showinfo("MANUAL SCRIPTS", "An error in execution of script has occurred!\nError Code:" + str(e))
            else:
                messagebox.showinfo("MANUAL SCRIPTS", "Script was sucessfully executed!")


    # Event binding for changing a selection in Manual Scripts Listbox
    # Tkinter passes an event object to onselect()
    def onSelectScript(self, event):
        # Show script command in label below Listbox when selection in Listbox is changed
        w = event.widget
        # Get selected item (string)
        script_label = w.get(w.curselection())
        # Get dict() value for key (selected item) from class variable self.manual_scripts_dict
        script_command = self.manual_scripts_dict[script_label]
        # Update label with script command (value) based on Listbox selected item (key)
        # label-text will be used later on for script execution
        self.manual_scripts_command["text"] = self.manual_scripts_dict[script_label]


    # Modal window to show error messages from Python scripts
    def modalErrors(self):
        # Read Errors from txt file
        with open("error-detection.txt", "r") as myfile:
            data = myfile.read()

        # Toplevel widget displayed in a separate, top-level window.
        self.win = tk.Toplevel()  # make a new window
        Framet1 = tk.Frame(self.win, bg="#e6e6e6", padx=10, pady=10)
        Framet1.grid(row=0, column=0, rowspan=3, columnspan=1, sticky="nsew")
        self.error_label = tk.Label(Framet1, bg="#e6e6e6", font=self.headbold, text='LIST OF ERRORS FROM HTOB SCRIPTS')  # add a few widgets
        self.error_label.grid(column=0, row=0, sticky="w")
        # be sure to call quit instead of destroy in dialog callback handlers
        self.error_text = tk.Text(Framet1, height=10, width=60)
        self.error_text.grid(column=0, row=1, sticky="w")
        self.error_text.insert(tk.END, data)
        self.error_text_scrollbar = tk.Scrollbar(Framet1)  # height= not permitted here!
        self.error_text.config(yscrollcommand=self.error_text_scrollbar.set)
        self.error_text_scrollbar.config(command=self.error_text.yview)
        self.error_text_scrollbar.grid(column=1, row=1, sticky='nsw')
        self.close_modal_btn = tk.Button(Framet1, text='Close Window', command=self.win.destroy)  # set quit callback
        self.close_modal_btn.grid(column=0, row=2, sticky="w")
        # WM = Windows Manager = closing the Window with "x" button in top Win-toolbar
        # WM_DELETE_WINDOW, and is used to define what happens when the user explicitly closes a window using the window manager.
        self.win.protocol('WM_DELETE_WINDOW', self.win.destroy)  # quit on wm close with callback function
        self.win.focus_set()  # take over input focus to modal window
        self.win.grab_set()  # disable other windows while modal window is open.
        self.win.mainloop()  # and start a nested event loop to wait for user interraction
